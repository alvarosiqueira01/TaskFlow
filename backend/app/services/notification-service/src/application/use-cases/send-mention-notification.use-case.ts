...
import { MentionCreatedEventPayload } from '../../events/consumed/mention-created.event-schema.js';
...
export class SendMentionNotificationUseCase {
  ...
  async execute(payload: MentionCreatedEventPayload): Promise<void> {
    const recipient = await this.userDirectoryRepository.findById(payload.mentionedUserId);
    if (!recipient) {
      throw new RecipientNotFoundError(payload.mentionedUserId);
    }
    if (!recipient.email) {
      throw new RecipientHasNoEmailError(payload.mentionedUserId);
    }

    const subject = 'You were mentioned in a comment';
    const body = `Hi ${recipient.fullName}, you were mentioned in a comment on task ${payload.taskId}.`;
